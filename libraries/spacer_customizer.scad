//resolutuion. Minimum angle for a fragment
 $fa=12;
 
// Minimum size of a fragment.
 $fs=2;

// Make either thread or hole
threads = 0; 

// Thread in both ends
both_ends = 0;

// How much space should be added to the hole/threads
thread_tolerance = 0.1;

// Metric tread outer diameter
top_thread_diameter = 8;

// Distance between peaks
top_thread_pitch = 1.25;

// How deep the thread goes
top_thread_depth = 4;
...
// Distance between peaks
bottom_thread_pitch = 1.5;

// How deep the thread goes
bottom_thread_depth = 4;
...
difference() {
    ...
    //Makes id if threads is false
    if  (threads<1)
        cylinder(d=id, h=length + 5, center=true);
    
    else 
        // Makes bottom thread
        translate([0,0,-length / 2])
        metric_thread(top_thread_diameter, top_thread_pitch + thread_tolerance, top_thread_depth, internal=true);
    
        // Makes top thread if needed
        if ((both_ends>0)&&(top_thread_depth+bottom_thread_depth<length))
            translate([0,0,length / 2 - bottom_thread_depth])
        metric_thread(bottom_thread_diameter + thread_tolerance, bottom_thread_pitch, bottom_thread_depth, internal=true);
}
