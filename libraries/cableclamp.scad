...
// Calculated values
ssd = od*0.6-2; // set screw diameter

//include <Nut_Job_Lib.scad>;
difference(){
union() {
difference(){
union (){
    parte_externa();
    translate([0,0,id]) parte_interna();
}
internal_hole();
}
inner_texture();
}
#translate([0,id/2+od/2+4,0.5*id]) rotate([90,0,0]) set_screw(ssd+0.7*tol);
}
translate([1.7*od,0,0]) nut(od+tol);
translate([0,1.3*od,0]) set_screw(ssd-0.3*tol);

/////////////////////////////////////////////////////////////////////////

module parte_externa() {
linear_extrude(height=id) {
hull(){
circle(d=od+2);
translate([0,od*0.4])square(od*0.6, center=true);}
}
}


module parte_interna() {translate([0,0,0]) hex_screw(od,tp,step_shape_degrees,id,resolution,countersink,head_diameter,0,non_thread_length,non_thread_diameter);}

module internal_hole () {
    translate([0,0,-1]) cylinder(h=2*od, d=id);
}

module set_screw (d) {
//    difference(){
//    hex_screw(d,ssp,step_shape_degrees,od*0.6,resolution,countersink,head_diameter,0,non_thread_length,non_thread_diameter);
//    translate([0,0,od*0.6-1]) cube([1,2.5,2], center=true);}
    socket_screw(d,ssp,step_shape_degrees,od*0.6,resolution,countersink,d-1,head_height,non_thread_length,non_thread_diameter);
}

module nut (d) {
    hex_nut(od+5,5,tp,nut_step_shape_degrees,d,nut_resolution);
}

module inner_texture () {
for (h = [0:1:2*id]) {
translate([0,0,h])rotate_extrude (angle=360) {
translate([id/2,0,0]) rotate([0,0,180])circle(d=0.6, $fn=3);}
}
}
